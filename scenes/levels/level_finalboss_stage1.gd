...
func _ready():
	...
	if TranslationServer.get_locale() == "ru":
		...
		get_parent().get_node("Heroes").current_character.get_node("CharacterCamera/Control/Objective").text = "Objective: Defeat Olgwasilna"
...
func _on_Node2D_died():
	get_parent().get_tree().change_scene("res://scenes/levels/cutscene_laststory_2.tscn")
